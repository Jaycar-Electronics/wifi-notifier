...
void setup()
{
  ...
  while ((!DEBUG) && (millis() < 10000))
    ;         //wait for serial port, or start anyway after 10 seconds
...
void loop()
{
  ...
  while (DEBUG.available())
  {
    int d = DEBUG.read();
    if (d == 13)
    { //if enter pressed, trigger email
      doemail();
    }
  }
  if ((coretemp > 35) && (rpttimeout + RPTDELAY < millis()))
  { //if it's been more than delay since last report
    doemail();
    rpttimeout = millis();
  }
}
...
int processcmd()
{
  ...
  if (!c)
  {
    DEBUG.println("Could not connect");
    return 0;
  ...
  if (!WIFIreceive("354"))
  {
    WIFIclose;
    return 0;
  }                                 // 354 means server is ready to receive content
  ...
  if (!WIFIreceive("250"))
  {
    WIFIclose;
    return 0;
  }                     //    250 here means mail successfully sent
  WIFIsend("QUIT\r\n"); //close connection
  if (!WIFIreceive("221"))
  {
    WIFIclose;
    return -1;
  } //  221 means connection closed cleanly, so fail here is not a problem as long as email was sent
  WIFIclose();
  return 1;
}
...
int WIFIreceive(char *s)
{ //wait for received ipd data to match string s
  int k = 0;
  long t;
  t = millis();
  while (millis() - t < 2000)
...
int WIFIsend(char *s)
{ //send string s in single connection mode
  long t;
